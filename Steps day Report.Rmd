...
```
- with impute now it can see been that distribution steps is normal
```

Now Compute new median and mean and compart both values 
...
Now compute steps by week day and weekend, this will be convert all column in weekday, and so, use condition ifelse and looking values when weekday was Saturday or Sunday and change value to weekend
...
```
- In weekday is more frequent intervals  500 to 900 than weekend, but the weekend is more frequent to make intervals more long that the weekday.

- Is necessary impute values NA beacuse it disturb distribution data.
